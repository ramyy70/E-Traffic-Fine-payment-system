    ...
    return (
        <div className="page-container">
            <section className="page-header">
                <h1 className="page-title">{t('trafficControlUnit')}</h1>
                <p className="page-subtitle">{t('policeDashboardSub')}</p>
            </section>
                ...
                <>
                    <div className="grid gap-3 sm:grid-cols-3">
                        <div className="stat-card">
                            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{t('totalFines')}</p>
                            <p className="mt-2 text-3xl font-extrabold text-slate-900">{fines.length}</p>
                        </div>
                        <div className="stat-card">
                            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{t('pending')}</p>
                            <p className="mt-2 text-3xl font-extrabold text-slate-900">
                                {fines.filter((fine) => fine.status === 'Unpaid').length}
                            </p>
                        </div>
                        <div className="stat-card">
                            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{t('statusPaid')}</p>
                            <p className="mt-2 text-3xl font-extrabold text-slate-900">
                    ...
                    <div className="surface-card p-0">
                        <div className="border-b border-slate-100 px-6 py-4">
                            <h2 className="text-lg font-bold text-slate-900">{t('recentIssuedFines')}</h2>
                            <p className="mt-1 text-sm text-slate-500">{t('recentIssuedFinesSub')}</p>
                        </div>

                        {recentFines.length === 0 ? (
                            <div className="px-6 py-12 text-center text-sm text-slate-500">{t('noFinesAvailable')}</div>
                        ) : (
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                                                ...
                                                <span className="inline-flex items-center gap-1">
                                                    ...
                                                    {fine.vehicleNo || t('vehicleNotSet')}
                                                </span>
                                            </div>
                                        </div>
