...
const reasonLabelKeys = {
    'Unfair Fine': 'reasonUnfairFine',
    'Officer Misconduct': 'reasonOfficerMisconduct',
    ...
    return (
            <div className="surface-card h-full p-0">
                <div className="border-b border-slate-100 px-6 py-4">
                    ...
                    <p className="mt-1 text-sm text-slate-500">{t('complaintListSub')}</p>
                </div>
                <div className="divide-y divide-slate-100">
                    {complaints.length === 0 ? (
                        <div className="p-8 text-center text-sm text-slate-500">{t('noComplaintsYet')}</div>
                    ) : (
                        complaints.map((complaint) => (
                            <div key={complaint.id} className="p-5 transition hover:bg-slate-50">
                                <div className="mb-2 flex items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-slate-500">
                                            {t('submittedOn')} {complaint.submittedDate}
                                        </p>
                                    </div>
                                ...
                                <p className="text-sm font-semibold text-slate-700">
                                    {t(reasonLabelKeys[complaint.reason] || complaint.reason)}
                                </p>
                                <div className="mt-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600">
                                    {complaint.description || t('noAdditionalDescription')}
                                </div>
                            </div>
                        ))
                )}
