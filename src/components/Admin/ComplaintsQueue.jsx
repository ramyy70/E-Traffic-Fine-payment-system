    ...
    return (
        <div className="surface-card p-0">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-4">
                <h3 className="text-lg font-bold text-slate-900">Complaints Queue</h3>
                <span className="badge-pill bg-amber-100 text-amber-700">{pendingComplaints.length} Pending</span>
            </div>

            {pendingComplaints.length === 0 ? (
                <div className="p-10 text-center text-sm text-slate-500">No pending complaints to review.</div>
            ) : (
                        ...
                        <div key={complaint.id} className="p-6 transition hover:bg-slate-50">
                            <div className="flex flex-col justify-between gap-4 xl:flex-row">
                                <div className="max-w-2xl space-y-2">
                                    ...
                                    <p className="text-sm font-semibold text-slate-800">Reason: {complaint.reason}</p>
