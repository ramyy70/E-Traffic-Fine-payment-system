...
import ComplaintSystem from '../../components/driver/ComplaintSystem';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-gray-800">{t('dashboard.activeFines')}</h2>
                <span className={`${activeFines.length > 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'} font-bold px-3 py-1 rounded-full text-xs`}>
                  {activeFines.length} {t('dashboard.pending')}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {loading ? (
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm text-center">
                    <p className="text-gray-500">{t('dashboard.noActiveFines')}</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="mt-8">
              <h2 className="text-xl font-bold text-gray-800 mb-4">{t('dashboard.transactionHistory')}</h2>
          ...
          <div className="lg:col-span-1 space-y-8">
            <section className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              <h2 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">{t('dashboard.systemMessages')}</h2>
                ...
                {successNotif && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-xl animate-in slide-in-from-top-2 duration-500 shadow-sm">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-xs font-bold text-green-700 uppercase tracking-wider">{t('dashboard.smsConfirmation')}</p>
                    </div>
                    <p className="text-sm text-green-800 font-medium leading-relaxed">
                      {t('nav.brandName')}: Payment for {t('dashboard.fineId')} {successNotif.id.split('-')[0].toUpperCase()} of {t('common.currency')} {successNotif.amount} was successful. Thank you.
                    </p>
                    <p className="text-[10px] text-green-600 mt-2 font-bold">{new Date(successNotif.time).toLocaleTimeString()}</p>
                  </div>
                )}
                <div className="p-4 bg-yellow-50 border border-yellow-100 rounded-xl">
                  <p className="text-sm text-yellow-800 font-medium">{t('dashboard.notifications.licenseExpiry')}</p>
                  <p className="text-xs text-yellow-600 mt-2">{t('dashboard.notifications.twoDaysAgo')}</p>
                </div>
                <div className="p-4 bg-green-50 border border-green-100 rounded-xl">
                  <p className="text-sm text-green-800 font-medium">{t('dashboard.notifications.driveSafe')}</p>
                  <p className="text-xs text-green-600 mt-2">{t('dashboard.notifications.oneWeekAgo')}</p>
                </div>
              </div>
            </section>
